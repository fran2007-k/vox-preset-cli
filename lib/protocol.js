...
const PEDAL1_PARAMS = {
  ...
  CHORUS: {
    // Range verified against the protocol doc's live-dial table (Pedal 1
    // Chorus Speed: 0x0064-0x4e10), inverted through the jump-quirk formula.
    speedHz: { dial: 1, liveDial: 0, kind: 'frequency', default: 0.1, min: 0.1, max: 10.0 },
...
const PEDAL2_PARAMS = {
  FLANGER: {
    // Doc range 0x0064-0x2708 -> semantic 0.1-5.0Hz.
    speedHz: { dial: 1, liveDial: 0, kind: 'frequency', default: 0.1, min: 0.1, max: 5.0 },
  ...
  BLK_PHASER: {
    // Doc range 0x0064-0x4e10 -> semantic 0.1-10.0Hz (same as Chorus's).
    speedHz: { dial: 1, liveDial: 0, kind: 'frequency', default: 0.1, min: 0.1, max: 10.0 },
  ...
  TREMOLO: {
    // Doc range 0x0c72-0x4e10 -> semantic 1.65-10.0Hz.
    speedHz: { dial: 1, liveDial: 0, kind: 'frequency', default: 1.65, min: 1.65, max: 10.0 },
    depth: { dial: 2, liveDial: 1, kind: 'unitless', default: 5.0 },
  ...
  TAPE_ECHO: {
    // Doc range 0x001e-0x0930 -> semantic 30-1200ms.
    timeMs: { dial: 1, liveDial: 0, kind: 'duration', default: 30, min: 30, max: 1200 },
...
const REVERB_PARAMS = {
  ...
  preDelayMs: { dial: 3, liveDial: 2, kind: 'durationByte', default: 0, min: 0, max: 100 },
...
// --- Single-field live control for Pedal 1 / Pedal 2 / Reverb -----------
// Mirrors buildAmpDialLiveMessage's one-field-at-a-time approach, but for
// the three effect slots -- so a "Live Pedal" UI can turn one knob, flip one
// footswitch, or switch one type, without resending an entire preset (that's
// still what buildLiveApplyMessages is for).

const PEDAL_SLOT_INFO = {
  pedal1: { typesTable: PEDAL1_TYPES, typeSlot: TYPE_CHANGE_PEDAL_SLOT.PEDAL1, liveSlot: LIVE_PEDAL_SLOT.PEDAL1 },
  pedal2: { typesTable: PEDAL2_TYPES, typeSlot: TYPE_CHANGE_PEDAL_SLOT.PEDAL2, liveSlot: LIVE_PEDAL_SLOT.PEDAL2 },
  reverb: { typesTable: REVERB_TYPES, typeSlot: TYPE_CHANGE_PEDAL_SLOT.REVERB, liveSlot: LIVE_PEDAL_SLOT.REVERB },
};

function pedalSlotInfo(slot) {
  const info = PEDAL_SLOT_INFO[slot];
  if (!info) throw new Error(`unknown pedal slot "${slot}", expected one of ${Object.keys(PEDAL_SLOT_INFO).join(', ')}`);
  return info;
}

/** The param spec table for one slot's currently-selected type (reverb's is the same regardless of type). */
function pedalParamsSpec(slot, type) {
  if (slot === 'reverb') return REVERB_PARAMS;
  const info = pedalSlotInfo(slot);
  const table = slot === 'pedal1' ? PEDAL1_PARAMS : PEDAL2_PARAMS;
  const spec = table[type];
  if (!spec) throw new Error(`unknown ${slot} type "${type}", expected one of ${Object.keys(info.typesTable).join(', ')}`);
  return spec;
}

/** Live "Pedal/Reverb Enabled" message for one slot. */
function buildPedalEnabledLiveMessage(slot, enabled) {
  const info = pedalSlotInfo(slot);
  return sysex(0x30, 0x00, 0x01, 0x34, 0x41, 0x02, info.typeSlot, enabled ? 0x01 : 0x00, 0x00);
}

/** Live "Type Changed" message for one slot. */
function buildPedalTypeLiveMessage(slot, type) {
  const info = pedalSlotInfo(slot);
  if (!(type in info.typesTable)) throw new Error(`unknown ${slot} type "${type}", expected one of ${Object.keys(info.typesTable).join(', ')}`);
  return sysex(0x30, 0x00, 0x01, 0x34, 0x41, 0x03, info.typeSlot, info.typesTable[type], 0x00);
}

/** Live "Dial Turned" message for one named param of one slot's current type. */
function buildPedalParamLiveMessage(slot, type, paramName, value) {
  const info = pedalSlotInfo(slot);
  const spec = pedalParamsSpec(slot, type);
  const paramSpec = spec[paramName];
  if (!paramSpec) throw new Error(`unknown ${slot} (${type}) param "${paramName}", expected one of ${Object.keys(spec).join(', ')}`);
  const semantic = resolveDialValue(paramSpec, value);
  return sysex(0x30, 0x00, 0x01, 0x34, 0x41, info.liveSlot, paramSpec.liveDial, ...encodeTwoByteDialStandard(semantic));
}

/**
 * {label, message} pairs that reset every param of one slot's type to that
 * type's documented defaults -- used when a type is switched live, so the
 * new type starts from a known-good sound instead of inheriting whatever
 * raw dial values the previous type happened to leave behind.
 */
function buildPedalDefaultsLiveMessages(slot, type) {
  const spec = pedalParamsSpec(slot, type);
  return Object.entries(spec).map(([name, paramSpec]) => ({
    label: `${slot}.params.${name}`,
    message: buildPedalParamLiveMessage(slot, type, name, paramSpec.default),
  }));
}
...
module.exports = {
  ...
  buildPedalEnabledLiveMessage,
  buildPedalTypeLiveMessage,
  buildPedalParamLiveMessage,
  buildPedalDefaultsLiveMessages,
  AMP_LIVE_DIAL,
  isAck,
};
