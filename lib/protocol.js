...
function buildLiveApplyMessages(preset) {
  const messages = [];
  const w = (...payloadBytes) => messages.push(sysex(...payloadBytes));
  ...
  // -- amp model --
  w(0x30, 0x00, 0x01, 0x34, 0x41, 0x03, 0x00, ampModel.byte, 0x00);

  // -- amp dials --
  const ampDialSemantic = (key) => {
    switch (key) {
      case 'gain': case 'treble': case 'middle': case 'bass': case 'volume':
      case 'presence': case 'resonance':
        return encodeUnitless(amp[key]);
      case 'brightCap': case 'lowCut': case 'midBoost':
        return amp[key] ? 0x01 : 0x00;
      case 'tubeBias':
        if (!(amp.tubeBias in TUBE_BIAS)) throw new Error(`unknown tubeBias "${amp.tubeBias}"`);
        return TUBE_BIAS[amp.tubeBias];
      case 'ampClass':
        if (!(amp.ampClass in AMP_CLASS)) throw new Error(`unknown ampClass "${amp.ampClass}"`);
        return AMP_CLASS[amp.ampClass];
      default:
        throw new Error(`unknown amp field ${key}`);
    }
  };
  for (const [key, dialId] of Object.entries(AMP_LIVE_DIAL)) {
    w(0x30, 0x00, 0x01, 0x34, 0x41, 0x04, dialId, ...encodeTwoByteDialStandard(ampDialSemantic(key)));
  }

  // -- noise reduction sensitivity (single byte, not 2-byte encoded) --
  w(0x30, 0x00, 0x01, 0x34, 0x41, 0x01, 0x00, encodeUnitless(amp.noiseReductionSensitivity), 0x00);

  function applyPedal(pedalConfig, typesTable, paramsTable, typeSlot, liveSlot) {
    w(0x30, 0x00, 0x01, 0x34, 0x41, 0x02, typeSlot, pedalConfig.enabled ? 0x01 : 0x00, 0x00);
    w(0x30, 0x00, 0x01, 0x34, 0x41, 0x03, typeSlot, typesTable[pedalConfig.type], 0x00);
    ...
    for (const [name, paramSpec] of Object.entries(spec)) {
      ...
      w(0x30, 0x00, 0x01, 0x34, 0x41, liveSlot, paramSpec.liveDial, ...encodeTwoByteDialStandard(semantic));
    }
  }

  applyPedal(pedal1, PEDAL1_TYPES, PEDAL1_PARAMS, TYPE_CHANGE_PEDAL_SLOT.PEDAL1, LIVE_PEDAL_SLOT.PEDAL1);
  applyPedal(pedal2, PEDAL2_TYPES, PEDAL2_PARAMS, TYPE_CHANGE_PEDAL_SLOT.PEDAL2, LIVE_PEDAL_SLOT.PEDAL2);

  w(0x30, 0x00, 0x01, 0x34, 0x41, 0x02, TYPE_CHANGE_PEDAL_SLOT.REVERB, reverb.enabled ? 0x01 : 0x00, 0x00);
  w(0x30, 0x00, 0x01, 0x34, 0x41, 0x03, TYPE_CHANGE_PEDAL_SLOT.REVERB, REVERB_TYPES[reverb.type], 0x00);
  for (const [name, paramSpec] of Object.entries(REVERB_PARAMS)) {
    ...
    const semantic = resolveDialValue(paramSpec, raw);
    w(0x30, 0x00, 0x01, 0x34, 0x41, LIVE_PEDAL_SLOT.REVERB, paramSpec.liveDial, ...encodeTwoByteDialStandard(semantic));
  }
